...
/// [`NormalLoader`] for a Qwen3.5 dense text model (`Qwen3_5ForCausalLM`).
///
/// Hybrid architecture: full-attention + GatedDeltaNet linear-attention, dense MLP.
...
impl DeviceMappedModelLoader for Qwen3_5Loader {
    fn mapped_max_act_size_elems(
        ...
        let AutoDeviceMapParams::Text {
            ...
            anyhow::bail!("Expected text AutoDeviceMapParams for Qwen3.5!")
        };
        let cfg: crate::models::qwen3_5::Config = serde_json::from_str(config)?;
    ...
    fn layer_sizes_in_bytes(
        ...
        let per_layer_elems = {
            ...
            let mlp = 3 * cfg.hidden_size * inter / weight_pack_factor;
...
/// [`NormalLoader`] for the Qwen3.5 MoE text model (`Qwen3_5MoeForCausalLM`).
///
/// Same hybrid architecture as Qwen3.5 dense but with sparse MoE feed-forward blocks.
...
impl DeviceMappedModelLoader for Qwen3_5MoeLoader {
    fn mapped_max_act_size_elems(
        ...
        let AutoDeviceMapParams::Text {
            ...
            anyhow::bail!("Expected text AutoDeviceMapParams for Qwen3.5-MoE!")
        };
        let cfg: crate::models::qwen3_5::Config = serde_json::from_str(config)?;
        Ok(max_batch_size * cfg.num_attention_heads * max_seq_len.min(&ATTENTION_CHUNK_SIZE).pow(2))
    }
    ...
    fn layer_sizes_in_bytes(
        ...
        let per_layer_elems = {
            ...
            let shared_expert =
                3 * cfg.hidden_size * cfg.shared_expert_intermediate_size / weight_pack_factor;
            let routed_experts =
                cfg.num_experts * 3 * cfg.hidden_size * cfg.moe_intermediate_size / weight_pack_factor;
            input_layernorm
                + post_attention_layernorm
                + q_proj
                + k_proj
